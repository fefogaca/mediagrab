...
const Tooltip = ({
  ...
  const positionOuterClasses = (position?: 'top' | 'right' | 'bottom' | 'left') => {
    switch (position) {
      case 'right':
  ...
  const sizeClasses = (size?: 'lg' | 'md' | 'sm') => {
    switch (size) {
      case 'lg':
  ...
  const colorClasses = (bg?: 'light' | 'dark') => {
    switch (bg) {
      case 'light':
  ...
  const positionInnerClasses = (position?: 'top' | 'right' | 'bottom' | 'left') => {
    switch (position) {
      case 'right':
        return 'ml-2';
      case 'left':
