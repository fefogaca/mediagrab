...
import { ThemeContext, useThemeProvider } from '@/app/components/ThemeProvider';
...
export default function ThemeToggle() {
  ...
  return (
    <div className="inline-flex">
      <button
        type="button"
        onClick={() => changeCurrentTheme(isLight ? "dark" : "light")}
        className="relative flex items-center justify-center cursor-pointer w-8 h-8 hover:bg-gray-100 lg:hover:bg-gray-200 dark:hover:bg-gray-700/50 dark:lg:hover:bg-gray-800 rounded-full transition-colors"
        aria-label="Toggle theme"
      >
        {/* Sun icon */}
        <svg
          className={`absolute transition-opacity duration-200 ${isLight ? 'opacity-100' : 'opacity-0'} fill-current text-gray-900`}
          width={16}
          height={16}
          viewBox="0 0 16 16"
